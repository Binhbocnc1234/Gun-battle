    ...
    public override void Shot(){
        if (m_gun.fireRate.isEnd && m_gun.reloadTime.isEnd){
            ...
            for(int i = 0; i < bulletPerShot; ++i){
                Bullet bul = m_gun.SpawnBullet(m_gun.owner.transform.position + m_gun.owner.transform.rotation.x*Gun.shotdiff, m_gun.owner.transform.rotation);
                bul.transform.Rotate(new Vector3(0,0, Random.Range(-m_gun.inaccuracy, +m_gun.inaccuracy)));
            }

            m_gun.SpawnBullet(m_gun.owner.transform.position + m_gun.owner.transform.rotation.x*Gun.shotdiff, m_gun.owner.transform.rotation);
